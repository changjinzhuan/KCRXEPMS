package cn.kcrxorg.kcrxepms.pasmutil;
...
public class PsamCmdUtil {
    ...
    public byte[] getPSAMinfo(int psam , PsamError err) {
        byte[] result = new byte[256] ;
        byte[] info = null ;
        ...
        if (len > 0) {
            ...
            if(resultStr.endsWith(RES_OK)){
                ...
                //用户ID
                System.arraycopy(result, 0,info, 0, 4);
    ...
    public byte[] getPSAMCertifi(int psam , PsamError err) {
    ...
    public byte[] getPSAMCertifiPart1(int psam , PsamError err)
    {
        byte[] result = new byte[512];
        byte[] certifipart1 = new byte[256];
        byte[] cmd = Tools.HexString2Bytes("8010020080");
        int len = rfid.psamapdu(psam, cmd, result);
        if (len > 0)
        {
            String resultStr = Tools.Bytes2HexString(result, len);
            Log.e("kcrx","第一部分的第一部分长度"+resultStr.length()+":"+resultStr);
            if(resultStr.endsWith("6180"))
            {
                System.arraycopy(result, 0, certifipart1, 0, 128);
                cmd = Tools.HexString2Bytes("00C0000080");
                len = rfid.psamapdu(psam, cmd, result);
                if (len > 0)
                {
                    resultStr = Tools.Bytes2HexString(result, len);
                    Log.e("kcrx","第一部分的第二部分长度"+resultStr.length()+":"+resultStr);
                    System.arraycopy(result,0,certifipart1,128,128);
                }
            }
        }
      return  certifipart1;
    }
    //获取PSAM数据证书第二部分  8010020000
    public byte[] getPSAMCertifiPart2(int psam , PsamError err)
    {
        byte[] result = new byte[512];
        byte[] certifipart2 = new byte[108];
        byte[] cmd = Tools.HexString2Bytes("8010020136");
        int len = rfid.psamapdu(psam, cmd, result);
        if (len > 0)
        {
            String resultStr = Tools.Bytes2HexString(result, len);
            if(resultStr.endsWith("6134"))
            {
                System.arraycopy(result, 0, certifipart2, 0, 54);
                cmd = Tools.HexString2Bytes("00C0000034");
                len = rfid.psamapdu(psam, cmd, result);
                if (len > 0)
                {
                    resultStr = Tools.Bytes2HexString(result, len);
                    System.arraycopy(result,0,certifipart2,54,54);
                }
            }
        }
        return  certifipart2;
    ...
    public boolean verifyUser(int psam, byte[] userID, byte[] userPIN, PsamError err){
        byte[] result = new byte[256] ;
        boolean flag = false ;
    ...
    public byte[] genTagActiveElsCmd(int psam, byte[] epc, byte volume, PsamError err) {
        byte[] elsCmd = null;
        String dataStr ;
        byte[] data ;
        String volumeStr ;
    ...
    public byte[] genOpenElsCmd(int psam, byte[] epc, int number, byte[] operator1, byte[] operator2, String dateTime, PsamError err) {
        byte[] elsCmd = null;
        String dataStr ;
        byte[] data ;
        byte[] dateTimeByte ;
    ...
    public byte[] genCloseElsCmd(int psam, byte[] epc, int number, byte[] operator1, byte[] operator2, String dateTime, PsamError err) {
        byte[] elsCmd = null;
        String dataStr ;
        byte[] data ;
        byte[] dateTimeByte ;
    ...
    //开锁写物流指令0x05
    //数据(交互指令DATA):张数（2字节）+ 操作员1(4字节) + 操作员2(4字节) + 操作时间(4字节) + 手持机ID(PSAM卡ID后20bit) + 位置序号(4bit) + 随机数(4字节)+保留(1字节)
    public byte[] genOpenWriteElsCmd(int psam, byte[] epc, int number, byte[] operator1, byte[] operator2, String dateTime, PsamError err) {
        byte[] elsCmd = null;
        String dataStr ;
        byte[] data ;
        byte[] dateTimeByte ;
    ...
    //关锁写物流指令0x04
    //张数（2字节）+ 操作员1(4字节) + 操作员2(4字节) + 操作时间(4字节) + 手持机ID(PSAM卡ID后20bit) + 位置序号(4bit) + 随机数(4字节)+保留(1字节)
    public byte[] genCloseWriteElsCmd(int psam, byte[] epc, int number, byte[] operator1, byte[] operator2, String dateTime, PsamError err) {
        byte[] elsCmd = null;
        String dataStr ;
        byte[] data ;
        byte[] dateTimeByte ;
    ...
    //恢复指令0x07
    //张数（2字节）+ 操作员1(4字节) + 操作员2(4字节) + 操作时间(4字节) + 手持机ID(PSAM卡ID后20bit) + 位置序号(4bit) + 随机数(4字节)+保留(1字节)
    public byte[] genRecoverElsCmd(int psam, byte[] epc, int number, byte[] operator1, byte[] operator2, String dateTime, PsamError err) {
        byte[] elsCmd = null;
        String dataStr ;
        byte[] data ;
        byte[] dateTimeByte ;
    ...
    //日志清除指令0x08
    //张数（2字节）+ 操作员1(4字节) + 操作员2(4字节) + 操作时间(4字节) + 手持机ID(PSAM卡ID后20bit) + 位置序号(4bit) + 随机数(4字节)+保留(1字节)
    public byte[] genClenLogElsCmd(int psam, byte[] epc, byte[] operator1, byte[] operator2, String dateTime, PsamError err) {
        byte[] elsCmd = null;
        String dataStr ;
        byte[] data ;
        byte[] dateTimeByte ;
    ...
    public byte[] genTraceElsCmd(int psam, byte[] epc, byte[] logId, byte[] logItem, byte[] logFlag , PsamError err){
        byte[] elsCmd = null;
        byte[] cmdType ;
        String dataStr ;
        byte[] data ;
    ...
    public byte[] genElsCmd(int psam, byte[] cmdType, byte[] epc, byte[] data, PsamError err){
        byte[] elsCmd = null ;
        byte[] result = new byte[256] ;
        String cmdStr ;
        byte[] cmd ;
    ...
    public byte[] genSignData(int psam, byte[] data, PsamError err){
        byte[] signData = null ;
        byte[] result = new byte[256] ;
        String cmdStr ;
        byte[] cmd ;
    ...
    //电子签封数据解密(DECRYPT_ELS_DATA)，只能解析追溯数据
    //8034 00 00 dataLen data
    public byte[] decryptElsData(int psam, byte[] epc, byte[] srcData, PsamError err) {
        byte[] deData = null ;
        byte[] result = new byte[256] ;
...
//                MyLogger.e(TAG, "getRFIDPassword result:" + Tools.Bytes2HexString(result, len));
    ...
    public byte[] getRFIDPassword(int psam, byte[] epc, PsamError err){
        byte[] result = new byte[256] ;
    ...
    public byte[] getAtr(int psam,PsamError err)
    {
        byte[] rs=new byte[32];
        int len= rfid.activeUcpu(rs);
        if(len>0)
        {
            String resultStr = Tools.Bytes2HexString(rs, len) ;
            MyLogger.e(TAG, "getAtr :" + resultStr);
        }
        return rs;
    }
    public String excute(int psam,String cmd,PsamError err)
    {
        byte[] result = new byte[256] ;
        ...
        int len = rfid.psamapdu(psam, cmdb, result);
